...
import {
    ...
    fetchDeletedArticle, fetchDeletedTag,
    recycleArticle, recycleTag,
    removeArticleFromDatabase, removeTagFromDatabase
} from "@/api";
...
function queryArticle() {
    loading.value = true
    fetchDeletedTag(page_conf.current, page_conf.size,
        keyword.value === '' ? undefined : keyword.value).then((res)=>{
        tagQueryData.length = 0
        page_conf.total = res.data.total
        Object.assign(tagQueryData, res.data.records)
    }).finally(()=>{
        loading.value = false
    })
}
...
function handleDelete(idList: string[]) {
    ElMessageBox.confirm(`真的要删除吗`,'提示', {
        ...
        removeTagFromDatabase(idList).then((res)=>{
            ElMessage.success(<string>res.data)
        }).finally(()=>{
            afterOperate()
        })
